...
(and (define a (* 1 2)) (lambda (x y) (* x y)))
; expect (lambda (x y) (* x y))
...
(cond ((and 1 2 3) (/ 1 0) (+ 1 2))
      (else (list 1 2 3)))
; expect Error: division by zero

(cond ((= (* 2 3) 7) 'Hello)
      ((odd? (and 1 2 3)) ((lambda (x y) (+ (expt x y) (expt y x))) 1 2))
      (else (cons 1 2)))
; expect 3
...
(define f (mu (x) (+ x y)))
(define g (lambda (x y) (f (+ (expt x y) (expt y x)))))
(g 3 7)
; expect 2537
...
;;; License: Creative Commons share alike with attributions
